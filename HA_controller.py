...
class Controller:
    ...
    def __init__(self, HA_acces_token, HA_port_nr = 8123):
        ...
        try:
            m1 = GET(self.url+"services", headers=self.headers).text
            m2 = GET(self.url+"states",   headers=self.headers).text
            self.services = json.loads(m1)
            self.states   = json.loads(m2)
        ...
        for i in range(len(self.states)):
            try:
                self.devices[ 
                    self.states[i]["attributes"]["friendly_name"] 
                    ] = self.states[i]
            except Exception as e:
                pass
